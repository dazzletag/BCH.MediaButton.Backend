...
export default function Dashboard({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  // Care-home assignment for the selected resident (admin-only edit)
  const [careHomes, setCareHomes] = useState<CareHome[]>([]);
  const [residentCareHomeId, setResidentCareHomeId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchCareHomes = async () => {
      try {
        const homes = await call<CareHome[]>({ url: "/api/admin/care-homes", method: "GET" });
        setCareHomes(homes || []);
      } catch (err) {
        console.error("Failed to load care homes", err);
      }
    };
    fetchCareHomes();
  }, [call, isAdmin]);

  const loadResidentCareHome = useCallback(async () => {
    const r = residentQuery.trim();
    if (!r) {
      setResidentCareHomeId(null);
      return;
    }
    try {
      const data = await call<{ careHomeId: string | null }>({
        url: `/api/admin/residents/${encodeURIComponent(r)}/care-home`,
        method: "GET",
      });
      setResidentCareHomeId(data?.careHomeId ?? null);
    } catch {
      setResidentCareHomeId(null);
    }
  }, [call, residentQuery]);

  const saveResidentCareHome = useCallback(async (newId: string | null) => {
    const r = residentQuery.trim();
    if (!r) return;
    ...
    try {
      await call({
        url: `/api/admin/residents/${encodeURIComponent(r)}/care-home`,
        method: "PUT",
        data: { careHomeId: newId },
      });
      setResidentCareHomeId(newId);
      const homeName = careHomes.find((h) => h.id === newId)?.name ?? "(none)";
      setSaveMessage(`Care home set to ${homeName} for ${r}.`);
    } catch (err) {
      console.error(err);
  ...
  }, [residentQuery, loadResidentCache, loadCacheCommands, loadAiPlaylist, loadResidentCareHome]);
        ...
        <div className="card glass">
          ...
          {isAdmin && residentQuery.trim() && (
            <div className="form-row" style={{ marginTop: 8 }}>
              <label className="muted" style={{ minWidth: 80, alignSelf: "center" }}>
                Care home:
              </label>
              <select
                className="select"
                disabled={savingCareHome}
                value={residentCareHomeId ?? ""}
                onChange={(e) => {
                  const newId = e.target.value || null;
                  if (newId === residentCareHomeId) return;
                  const homeName = newId
                    ? (careHomes.find((h) => h.id === newId)?.name ?? "(unknown)")
                    : "(none)";
                  if (window.confirm(`Move ${residentQuery.trim()} to ${homeName}?`)) {
                    saveResidentCareHome(newId);
                  } else {
                    // Reset the visible value back to current
                    e.target.value = residentCareHomeId ?? "";
                  }
                }}
              >
              ...
              {savingCareHome && <span className="muted">Saving…</span>}
            </div>
          )}
        </div>
